...
public class SoundKit : MonoBehaviour
{
	...
	public class SKSound
	{
		...
		private IEnumerator fadeIn( float duration, float targetVolume, Action onComplete )
		{
			// fade out the volume
			while( audioSource.volume < targetVolume && _elapsedTime < audioSource.clip.length )
			{
		...
		public void fadeOutAndStop( float duration, Action handler = null )
		{
			if(_fadeInCoroutine != null)
				_manager.StopCoroutine(_fadeInCoroutine);

			_fadeOutCoroutine = fadeOut( duration, () =>
			                            {
				if( handler != null )
					handler();
			});
		...
		public void fadeInMusic( float duration, float targetVolume, Action handler = null )
		{
			if(_fadeOutCoroutine != null)
			_manager.StopCoroutine(_fadeOutCoroutine);

			_fadeInCoroutine = fadeIn( duration, targetVolume, () =>
			                          {
				if( handler != null )
					handler();
			});
		...
		internal void playAudioClip( AudioClip audioClip, float volume, float pitch, float pan )
		{
			...
			audioSource.panStereo = pan; // Pan removed in version 5.2.1p2
#else
