...
[System.Serializable]
    public abstract class Node : ScriptableObject
    {
        ...
        public abstract Node Create (Vector2 pos, string name = "");
...
//              GUI.matrix = GUIMatrix;
        ...
        public void Delete ()
        {
            ...
            for (int outCnt = 0; outCnt < Outputs.Count; outCnt++)
            {
                ...
                {
                    RemoveConnection(output.connections[0], output);
                }

                GameObject.DestroyImmediate (output, true);
            }
            ...
            for (int inCnt = 0; inCnt < Inputs.Count; inCnt++)
            {
                ...
                {
//                  RemoveConnection(input.connections[0]);
                    input.connections.Remove(input.connections[0]);
                }

                GameObject.DestroyImmediate (input, true);
            }
        ...
        public bool allInputsReady ()
        {
            for (int inCnt = 0; inCnt < Inputs.Count; inCnt++)
            {
                for (int j = 0; j < Inputs[inCnt].connections.Count; j++)
                {
                    if (Inputs[inCnt].connections[j] == null || Inputs[inCnt].connections[j].IsValueNull)
                    {
                        return false;
                    }
                }
        ...
        public bool hasUnassignedInputs ()
        {
            for (int inCnt = 0; inCnt < Inputs.Count; inCnt++)
            {
                for (int j = 0; j < Inputs[inCnt].connections.Count; j++)
                {
                    if (Inputs [inCnt].connections[j] == null)
                    {
                        return true;
                    }
                }
        ...
        public bool descendantsCalculated ()
        {
            ...
            {
                for (int j = 0; j < Inputs[cnt].connections.Count; j++)
                {
                    if (Inputs [cnt].connections[j] != null && !Inputs [cnt].connections[j].body.calculated)
                    {
                        return false;
                    }
                }
        ...
        public bool isInput ()
        {
            for (int cnt = 0; cnt < Inputs.Count; cnt++)
            {
                for (int j = 0; j < Inputs[cnt].connections.Count; j++)
                {
                    if (Inputs [cnt].connections[j] != null)
                    {
                        return false;
                    }
                }
        ...
        public bool isChildOf (Node otherNode)
        {
            ...
            for (int cnt = 0; cnt < Inputs.Count; cnt++)
            {
                for (int j = 0; j < Inputs[cnt].connections.Count; j++)
                {
                    NodeOutput connection = Inputs [cnt].connections[j];
        ...
        public static bool CanApplyConnection (NodeOutput output, NodeInput input)
        {
            ...
            for (int j = 0; j < input.connections.Count; j++)
            {
                if (input.body == output.body || input.connections[j] == output)
                {
                    retVal = false;
                }
            }
...
//              input.connections.Clear();
        ...
        /// </summary>
        public static void RemoveConnection (NodeInput input, NodeOutput output)
        {
            Debug.Log(output);
            ...
            for (int j = 0; j < input.connections.Count; j++)
            {
                if(input.connections[j].connections != null)
                {
                    input.connections[j].connections.Remove (input);
                }
