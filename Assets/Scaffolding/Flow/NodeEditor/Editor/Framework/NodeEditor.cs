    ...
    public static class NodeEditor
    {
        ...
        public static void checkInit ()
        {
            if (!initiated && !InitiationError)
            {
                ...
                editorPath = ScaffoldingConfig.Instance.ScaffoldingPath + "/Flow/NodeEditor/Editor/";
...
//                              nodeOutput.connections.Clear();
                                ...
                                else
                                {
                                    ...
                                    for (int j = 0; j < nodeInput.connections.Count; j++)
                                    {
                                        if (nodeInput != null && nodeInput.connections[j] != null)
                                        {
                                            // Input node -> Loose and edit Connection
                                            curEditorState.connectOutput = nodeInput.connections[j];
                                            Node.RemoveConnection (nodeInput, nodeOutput);
                                            e.Use();
                                        }
                                    }
                                }
...
//                          #endif
        ...
        public static void SaveNodeCanvas (NodeCanvas nodeCanvas, string path, params NodeEditorState[] editorStates)
        {
            ...
            {
                Debug.LogError("Asset save path isnt set correctly: " + path);
            }
...
//          UnityEditor.AssetDatabase.Refresh ();
        ...
        public static NodeCanvas LoadViewCanvas()
        {
            NodeCanvas nodeCanvas = null;
//          for (int cnt = 0; cnt < objects.Length; cnt++)
//          {
        ...
        public static NodeCanvas GetWorkingCopy (NodeCanvas nodeCanvas)
        {
            ...
            for (int nodeCnt = 0; nodeCnt < nodeCanvas.nodes.Count; nodeCnt++)
            {
                ...
                for (int inCnt = 0; inCnt < node.Inputs.Count; inCnt++)
                {
                    ...
                    for (int j = 0; j < nodeInput.connections.Count; j++)
                    {
                        nodeInput.connections[j] = ReplaceSO (scriptableObjects, clonedScriptableObjects, nodeInput.connections[j]);
                    }
                }
